...
本项目实现一个可安装的 Kiro-style Spec Mode Skill，供 Codex、Claude Code 以及其他支持 Agent Skills / `SKILL.md` 的 CLI 工具使用。
...
文档输出目录不使用 Kiro 默认的 `.kiro/specs`，而是使用用户指定的文档管理根目录，并在其下按具体需求名分层；文档直接放在具体需求目录中，不再额外创建 `spec/` 子目录：
...
- **Spec Mode Skill**：以 `SKILL.md` 为入口的可安装 agent skill，用于执行 Kiro 风格的规格化开发流程。
...
4. THE Skill SHALL NOT 默认使用 `.kiro/specs`，除非用户显式要求 Kiro 原生目录。
5. THE Skill SHALL 在 spec 文件夹中保存 `.config.json`，记录 workflow type、spec type、需求名和路径。

---

### 需求 3：提供 Kiro 风格的阶段化文档生成流程
...
### 需求 4：模仿 Kiro 生成文档内容结构

**用户故事：** 作为 Kiro spec 用户，我希望生成文档的结构接近 Kiro 输出，以便在不同工具之间保持一致的阅读和执行体验。
...
5. THE Skill SHALL 参考 Kiro 样例中 `[ ]`、`[~]`、`[x]` 风格的任务状态表达。
...
3. THE Skill SHALL 避免声称复刻 Kiro 私有 IDE 实现，只复刻公开流程和可落地文件化工作流。
